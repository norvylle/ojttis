...
class ViewTwo extends Component{
	...
	componentDidMount() {
		fetch('http://localhost:3001/view-two')
		...
		<Table celled selectable inverted>
			<Table.Header>
				<Table.Row>
	          		...
	          		<Table.HeaderCell colspan="4" textAlign="center">Internship</Table.HeaderCell>
	          	</Table.Row>
			</Table.Header>
			<Table.Header>
				<Table.Row>
			...
			{this.state.student.map((item,index)=>{
				return(<Table.Row key={index}>
					...
					<Table.Cell>{item.company_name !== null ? item.company_name:"N/A"}</Table.Cell>
					<Table.Cell>{item.date_started !== null ? item.date_started:"N/A"}</Table.Cell>
					<Table.Cell>{item.no_of_hours_required	 !== null ? item.no_of_hours_required:"N/A"}</Table.Cell>
					<Table.Cell>{item.no_of_hours_worked !== null ? item.no_of_hours_worked:"N/A"}</Table.Cell>
				</Table.Row>)
			})}
			</Table.Body>
		</Table>
